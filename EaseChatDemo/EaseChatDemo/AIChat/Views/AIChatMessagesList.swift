...
@objc public protocol IAIChatMessagesListDriver: NSObjectProtocol {
    ...
    func editMessage(message: ChatMessage,finished: Bool)
    ...
    func refreshMessagePlayButtonState(message: MineMessageEntity)
}
...
@objc public protocol MessageListViewActionEventsDelegate: NSObjectProtocol {
    ...
    func onPlayButtonClick(message: MineMessageEntity)
}
...
open class AIChatMessagesList: UIView {
    ...
    public private(set) var messages: [MineMessageEntity] = [MineMessageEntity]()
    ...
    public required init(frame: CGRect, chatType: ChatType) {
        ...
        self.inputActions()
        self.moreMessages.setImage(UIImage(named: "more_messages"), for: .normal)
...
extension AIChatMessagesList:UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let entity = self.messages[safe: indexPath.row] else { return AIChatMessageCell() }
        ...
        guard let cell = self.registerMessageCell(tableView: tableView, indexPath: indexPath) else {
            return AIChatMessageCell(towards: direction, reuseIdentifier: "AIChat.MessageCell", chatType: self.chatType)
        }
...
//            ToastView.show(text: "already copy '\(textToCopy)' to pasteboard", postion: .center)
    ...
    func processBubbleClickAction(area: MessageCellClickArea, entity: MineMessageEntity) {
        switch area {
        case .bubble:
...
//            }
    ...
    private func convertMessage(message: ChatMessage,editFinished: Bool = false) -> MineMessageEntity {
        let entity = MineMessageEntity()
    ...
    private func registerMessageCell(tableView: UITableView,indexPath: IndexPath) -> AIChatMessageCell? {
        if let message = self.messages[safe: indexPath.row]?.message {
            ...
            case .text:
                var cell = tableView.dequeueReusableCell(with: AIChatTextMessageCell.self, reuseIdentifier: "AIChat.TextMessageCell")
                if cell == nil {
                    cell = AIChatTextMessageCell(towards: towards, reuseIdentifier: "AIChat.TextMessageCell", chatType: self.chatType)
                }
                return cell
            default:
                return nil
...
extension AIChatMessagesList: IAIChatMessagesListDriver {
    public func refreshMessagePlayButtonState(message: MineMessageEntity) {
        if let index = self.messages.firstIndex(where: { $0.message.messageId == message.message.messageId }) {
            if let cell = self.chatView.cellForRow(at: IndexPath(row: index, section: 0)) as? AIChatTextMessageCell {
    ...
    public func updateMessageStatus(message: ChatMessage, status: ChatMessageStatus) {
        if let index = self.messages.firstIndex(where: { $0.message.localTime == message.localTime }) {
            ...
            if let cell = self.chatView.cellForRow(at: IndexPath(row: index, section: 0)) as? AIChatMessageCell {
    ...
    private func showMessageAnimation(message: ChatMessage) {
    ...
    public func editMessage(message: ChatMessage,finished: Bool = false) {
    ...
    private func insertTypingMessage(to: String) {
        ...
        let message = ChatMessage(conversationID: to, body: ChatTextMessageBody(text: "•••"), ext: ["ai_chat":["user_meta":["botId":botId]]])
...
class ChatBotSelectCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        self.cover.image = UIImage(named: "Union")
    }
