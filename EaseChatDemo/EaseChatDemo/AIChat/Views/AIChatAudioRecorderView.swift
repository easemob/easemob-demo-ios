...
class AIChatAudioRecorderView: UIImageView {
    ...
    override init(frame: CGRect) {
        ...
        self.image = UIImage(named: "audio_recoder_bg")
...
//        SDImageCodersManager.shared.addCoder(APNGCoder)
    ...
    func setConstraints() {
        ...
        self.volumeIndicator.sd_setImage(with: nil, placeholderImage: UIImage(named: "No sound"), options: .retryFailed, context: nil)
...
//        aichatPrint("updateIndicatorImage: \(volume)")
        switch volume {
            ...
            self.volumeIndicator.sd_setImage(with: nil, placeholderImage: UIImage(named: "No sound"), options: .retryFailed, context: nil)
            default:
            if let url = Bundle.main.url(forResource: "with_sound", withExtension: "apng"),!self.playAPNG {
...
//            self.volumeIndicator.sd_setImage(with: url)
    ...
    func stopAPng() {
        self.volumeIndicator.sd_setImage(with: nil,
                                         placeholderImage: UIImage(named: "No sound"), 
                                         options: .retryFailed,
                                         context: nil)
        playAPNG = false
    }
    ...
    func refreshBackground(with state: LongPressButton.State) {
        ...
        case .start:
            ...
            self.recorderView.image = UIImage(named: "recording_bg")
        case .cancel:
            self.operationIndicator.text = "松手取消"
            self.recorderView.image = UIImage(named: "recording_bg_waring")
        case .end:
