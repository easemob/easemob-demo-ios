...
//        if let body = self.message.body as? ChatCustomMessageBody,body.event == "AIChat_alert_message" {
...
//        let text = (self.message.body as? ChatTextMessageBody)?.text ?? ""
//        if text.isEmpty {
//            return .zero
//        }
...
//            if let body = self.message.body as? ChatCustomMessageBody,body.event == "AIChat_alert_message",let something = self.message.ext?["something"] as? String {
//                if something.chat.numCount == 10 {
//                    return NSMutableAttributedString {
...
//    }
    ...
    var existTTSFile: Bool {
        FileManager.default.fileExists(atPath: (NSHomeDirectory())+"/\(self.messageId).mp3")
    }
    
    var isMarkdown: Bool {
        if let is_markdown = self.ext?["is_markdown"] as? Bool {
            return is_markdown
        }
        return true
    }
