...
class AIChatTextMessageCell: AIChatMessageCell {
    ...
    public required init(towards: BubbleTowards, reuseIdentifier: String, chatType: ChatType) {
        ...
        self.playButton.setImage(UIImage(named: "playing", ), for: .selected)
    ...
    override func refresh(entity: MineMessageEntity) {
        ...
        if entity.message.direction == .receive {
            if entity.editState == .typing {
                ...
                self.content.isHidden = true
                if let url = Bundle.main.url(forResource: "agents_msg_rply", withExtension: "apng") {
            ...
            if entity.editState == .end {
                ...
                if entity.playing {
                    ...
                    self.playButton.setImage(UIImage(named: "playing"), for: .normal)
                    self.entity.playing = true
                } else {
                    if entity.downloading {
                        self.playButton.setTitle(" 正在识别", for: .normal)
                        self.playButton.setImage(UIImage(named: "voice_spinner"), for: .normal)
                        self.addRotation()
