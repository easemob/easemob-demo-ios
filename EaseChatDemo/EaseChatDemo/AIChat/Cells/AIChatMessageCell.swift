...
@objcMembers open class AIChatMessageCell: UITableViewCell {
    
    private var longGestureEnabled: Bool = true
    
    public var entity = MineMessageEntity()
    
    public private(set) var towards = BubbleTowards.left
    
    public private(set) var chatType = ChatType.chat
        
    public var clickAction: ((MessageCellClickArea,MineMessageEntity) -> Void)?
        
    public var longPressAction: ((MessageCellClickArea,MineMessageEntity) -> Void)?
    ...
    required public init(towards: BubbleTowards,reuseIdentifier: String, chatType: ChatType) {
...
//                self.clickAction?(.checkbox,self.entity)
    ...
    open func refresh(entity: MineMessageEntity) {
        ...
        if let bot = entity.message.bot {
            self.avatar.sd_setImage(with: URL(string: bot.botIcon), placeholderImage: UIImage(named: "botavatar"), options: .retryFailed, context: nil)
        } else {
            self.avatar.image = UIImage(named: "botavatar")
        }
        //message status
    ...
    open func updateAxis(entity: MineMessageEntity) {
    ...
    @objc func updateMessageStatus(entity: MineMessageEntity) {
        self.status.image = entity.stateImage
        if entity.state != .sending {
            self.status.layer.removeAllAnimations()
 ...
 */
extension AIChatMessageCell: ThemeSwitchProtocol {
    /**
